export const seedBook = [
   {
      title: 'How to Win Friends & Influence People',
      author: 'Dale Carnegie',
      links: {
         audio: '',
         book: '',
      },
   },
   {
      title: 'The Things You Can See Only When You Slow Down',
      author: 'Haemin Sunim',
      links: {
         audio: '',
         book: '',
      },
   },
   {
      title: 'Funiculi Funicula',
      author: 'Toshikazu Kawaguchi',
      links: {
         audio: '',
         book: '',
      },
   },
   {
      title: 'The Psychology of Money',
      author: 'Morgan Housel',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Psychology-Money-Timeless-Lessons-Happiness/dp/B08D9WJ9G8',
      },
   },
   {
      title: 'The Courage to Be Disliked: How to Free Yourself, Change your Life and Achieve Real Happiness',
      author: 'Ichiro Kishimi, Fumitake Koga',
      links: {
         audio: '',
         book: '',
      },
   },
   {
      title: 'Ikigai',
      author: 'Héctor García',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Ikigai-audiobook/dp/B074WCX5X4/ref=sr_1_1?keywords=ikigai&qid=1673455593&s=audible&sprefix=ikiga%2Caudible%2C332&sr=1-1',
      },
   },
   {
      title: 'Filosofi Teras',
      author: 'Henry Manampiring',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Ikigai-audiobook/dp/B074WCX5X4/ref=sr_1_1?keywords=ikigai&qid=1673455593&s=audible&sprefix=ikiga%2Caudible%2C332&sr=1-1',
      },
   },
   {
      title: 'Rich Dad Poor Dad',
      author: 'Robert T. Kiyosaki',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Ikigai-audiobook/dp/B074WCX5X4/ref=sr_1_1?keywords=ikigai&qid=1673455593&s=audible&sprefix=ikiga%2Caudible%2C332&sr=1-1',
      },
   },
   {
      title: 'Retórica',
      author: 'Aristoteles Obras Completas',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Retorica-Aristoteles-Obras-Completas-ARISTOTELES/dp/8578275268',
      },
   },
   {
      title: 'The Subtle Art of Not Giving a F*ck',
      author: 'Aristoteles Obras Completas',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Retorica-Aristoteles-Obras-Completas-ARISTOTELES/dp/8578275268',
      },
   },
   {
      title: 'Sapiens: A Brief History of Humankind',
      author: 'Yuval Noah Harari',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Retorica-Aristoteles-Obras-Completas-ARISTOTELES/dp/8578275268',
      },
   },
   {
      title: 'Madilog: Materialisme, Dialektika, dan Logika',
      author: 'Tan Malaka',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Retorica-Aristoteles-Obras-Completas-ARISTOTELES/dp/8578275268',
      },
   },
   {
      title: 'Rusak Saja Buku Ini',
      author: 'Sony Adams',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Retorica-Aristoteles-Obras-Completas-ARISTOTELES/dp/8578275268',
      },
   },
   {
      title: 'The Ego and the Id',
      author: 'Sigmund Freud',
      links: {
         audio: '',
         book: 'https://www.amazon.com/Retorica-Aristoteles-Obras-Completas-ARISTOTELES/dp/8578275268',
      },
   },
];
